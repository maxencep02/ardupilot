...
void AP_MotorsHeli_Single::enable()
{
    ...
    rc_enable_ch(AP_MOTORS_MOT_4);    // swash servo 4
    rc_enable_ch(AP_MOTORS_MOT_5);    // swash servo 5
    rc_enable_ch(AP_MOTORS_MOT_6);    // swash servo 6
...
void AP_MotorsHeli_Single::calculate_roll_pitch_collective_factors()
{
    if (_swash_type == AP_MOTORS_HELI_SINGLE_SWASH_CCPM) {                     //CCPM Swashplate, perform control mixing
        ...
        // roll factors
        _rollFactor[CH_1] = cosf(radians(_servo1_pos + (_phase_angle + _delta_phase_angle)));
        _rollFactor[CH_2] = cosf(radians(_servo2_pos + (_phase_angle + _delta_phase_angle)));
        _rollFactor[CH_3] = cosf(radians(_servo3_pos + (_phase_angle + _delta_phase_angle)));
        _rollFactor[CH_4] = cosf(radians(_servo4_pos + (_phase_angle + _delta_phase_angle)));
        _rollFactor[CH_5] = cosf(radians(_servo5_pos + (_phase_angle + _delta_phase_angle)));
        _rollFactor[CH_6] = cosf(radians(_servo6_pos + (_phase_angle + _delta_phase_angle)));

        // pitch factors
        _pitchFactor[CH_1] = cosf(radians(_servo1_pos + 90 + (_phase_angle + _delta_phase_angle)));
        _pitchFactor[CH_2] = cosf(radians(_servo2_pos + 90 + (_phase_angle + _delta_phase_angle)));
        _pitchFactor[CH_3] = cosf(radians(_servo3_pos + 90 + (_phase_angle + _delta_phase_angle)));
        _pitchFactor[CH_4] = cosf(radians(_servo4_pos + 90 + (_phase_angle + _delta_phase_angle)));
        _pitchFactor[CH_5] = cosf(radians(_servo5_pos + 90 + (_phase_angle + _delta_phase_angle)));
        _pitchFactor[CH_6] = cosf(radians(_servo6_pos + 90 + (_phase_angle + _delta_phase_angle)));
...
uint16_t AP_MotorsHeli_Single::get_motor_mask()
{
    ...
    return rc_map_mask(1U << 0 | 1U << 1 | 1U << 2 | 1U << 3 | 1U << 4 | 1U << 5 | 1U << AP_MOTORS_HELI_SINGLE_AUX | 1U << AP_MOTORS_HELI_SINGLE_RSC);
}
...
//        yaw_offset = 0;  
    ...
    // swashplate servos
    _swash_servo_1.servo_out = (-_rollFactor[CH_1] * roll_out + _pitchFactor[CH_1] * pitch_out )/10 - 55.0/127.0 * yaw_out/15 + _collectiveFactor[CH_1] * coll_out_scaled + (_swash_servo_1.radio_trim-1500);
    _swash_servo_2.servo_out = (-_rollFactor[CH_2] * roll_out + _pitchFactor[CH_2] * pitch_out )/10 - 55.0/127.0 * yaw_out/15 + _collectiveFactor[CH_2] * coll_out_scaled + (_swash_servo_2.radio_trim-1500);
    _swash_servo_3.servo_out = (-_rollFactor[CH_3] * roll_out + _pitchFactor[CH_3] * pitch_out )/10 - 55.0/127.0 * yaw_out/15 + _collectiveFactor[CH_3] * coll_out_scaled + (_swash_servo_3.radio_trim-1500);   
    _swash_servo_4.servo_out = (_rollFactor[CH_4] * roll_out + _pitchFactor[CH_4] * pitch_out  )/10 + 30.0/127.0 * yaw_out/15 + _collectiveFactor[CH_4] * coll_out_scaled + (_swash_servo_4.radio_trim-1500);
    _swash_servo_5.servo_out = (_rollFactor[CH_5] * roll_out + _pitchFactor[CH_5] * pitch_out  )/10 + 30.0/127.0 * yaw_out/15 + _collectiveFactor[CH_5] * coll_out_scaled + (_swash_servo_5.radio_trim-1500);
    _swash_servo_6.servo_out = (_rollFactor[CH_6] * roll_out + _pitchFactor[CH_6] * pitch_out  )/10 + 30.0/127.0 * yaw_out/15 + _collectiveFactor[CH_6] * coll_out_scaled + (_swash_servo_6.radio_trim-1500);   
